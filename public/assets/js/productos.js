...
async function fetchProductos(categoryId = null) {
  ...
  return data.map((p) => {
    const first = p.first_image
      ? IMAGE_BASE + p.first_image.trim()
      : IMAGE_BASE + "no-image.png";
    const imgs = p.images
      ? p.images
          .split(",")
          .map((s) => IMAGE_BASE + s.trim())
          .filter(Boolean)
      : [first];
    return {
      id: Number(p.id),
...
function setupSliderNavigation() {
  ...
  rightBtn.onclick = () => {
    ...
    slider.scrollTo({
      left: Math.min(slider.scrollLeft + step, max),
      behavior: "smooth",
    });
  };
  leftBtn.onclick = () => {
    slider.scrollTo({
      left: Math.max(slider.scrollLeft - step, 0),
      behavior: "smooth",
    });
  };
}
  ...
  featured.forEach((p, i) => {
    ...
    slide.innerHTML = `
        <div class="slide-inner">
            ...
            <div class="slide-image"><img src="${p.image}" alt="${
      p.name
    }"></div>
        </div>`;
...
function nextSlide() {
  currentSlide = (currentSlide + 1) % $$(".dot").length;
  goToSlide(currentSlide);
}
function prevSlide() {
  currentSlide = (currentSlide - 1 + $$(".dot").length) % $$(".dot").length;
  goToSlide(currentSlide);
}
...
function renderProductos(filter = "all") {
  ...
  const list =
    filter === "all"
      ? products
      : products.filter(
          (p) => p.category_id == filter || p.category === filter
        );
  list.forEach((p) => {
    const first = p.image,
      second = p.images?.[1] || first;
    ...
    card.innerHTML = `
        <div class="product-image" data-action="open" data-id="${p.id}">
            ...
            <button class="btn-icon wishlist-btn ${
              isWish ? "active" : ""
            }" data-action="wish" data-id="${
      p.id
    }"><i class="bx bx-heart"></i></button>
            <button class="btn-icon cart-btn" data-action="cart" data-id="${
              p.id
        ...
        <div class="product-info">
            ...
            <button class="btn-view-detail" data-action="open" data-id="${
              p.id
...
function toggleWishlist(p) {
  ...
  if (i > -1) {
    wishlist.splice(i, 1);
    showNotification(`${p.name} eliminado`);
  } else {
    wishlist.push(p);
...
function renderWishlist() {
  ...
  if (!wishlist.length) {
    wrap.innerHTML = '<div class="empty-message">Lista vacía</div>';
    return;
  }
  wrap.innerHTML = wishlist
    .map(
      (i) => `
        ...
        <div class="wishlist-item-info"><div>${i.name}</div><div>${fmtMoney(
        i.price
      )}</div></div>
...
function renderCart() {
  const wrap = $("#cartItems"),
    totalEl = $("#cartTotal");
  if (!cart.length) {
    wrap.innerHTML = '<div class="empty-message">Carrito vacío</div>';
  ...
  wrap.innerHTML = cart
    .map(
      (i) => `
        ...
        <div class="cart-item-info"><div>${i.name}</div><div>${fmtMoney(
        i.price
      )}</div></div>
        <div class="cart-item-actions">
...
function showProductDetail(p) {
  ...
  $(
    "#specs"
  ).innerHTML = `<h2 class="text-2xl font-bold mb-6">Acerca del producto</h2>
  ...
  related.innerHTML = relatedList.length
    ...
    : `<p class="text-gray-500">No hay productos relacionados en esta categoría.</p>`;
...
function renderCheckoutSummary() {
  ...
  w.innerHTML =
    cart
      .map(
        (i) =>
          `<div class="co-item"><div>${i.name} x${
            i.quantity
          }</div><div>${fmtMoney(i.price * i.quantity)}</div></div>`
      )
      .join("") +
...
// ---------------------- Inicialización ----------------------
async function loadProductos(cat = "all") {
  products = await fetchProductos(cat);
}
async function init() {
  loadLocal();
