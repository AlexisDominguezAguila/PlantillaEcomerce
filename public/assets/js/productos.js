...
async function fetchProductos(categoryId = null) {
  ...
  return data.map(p => ({
    ...
    image: p.first_image ? IMAGE_BASE + p.first_image : IMAGE_BASE + "no-image.png",
    images: p.images ? p.images.split(",").map(img => IMAGE_BASE + img) : [IMAGE_BASE + (p.first_image || "no-image.png")]
  }));
}
...
function renderCategorias() {
  ...
  categories.forEach(c => {
    ...
    card.onclick = async ()=>{ await loadProductos(c.id); renderProductos(); };
  ...
  select.onchange = async (e)=>{
    await loadProductos(e.target.value);
    renderProductos();
  };
}
...
function showProductDetail(p){
  ...
  thumbs.innerHTML="";
  let imgs=p.images||[p.image];let idx=0;main.src=imgs[0];
  ...
  imgs.forEach((src,i)=>{const b=document.createElement("button");b.innerHTML=`<img src="${src}" class="w-20 h-20 object-cover">`;b.onclick=()=>{idx=i;main.src=src;};thumbs.appendChild(b);});
...
function renderPaymentMethodView(){
  ...
  if(selectedPaymentMethod==="wallet"){
    v.innerHTML=`<div class="wallet-box"><p>Monto: <strong>${totalText}</strong></p>
      <img src="${YAPE_QR}" class="wallet-qr-img"/><p>Enviar a: ${YAPE_PHONE}</p>
      <button id="walletConfirm" class="btn-primary w-full">Ya pagué</button></div>`;
    $("#walletConfirm").onclick=()=>confirmPayment("Yape/Plin",total);
  }else if(selectedPaymentMethod==="transfer"){
    v.innerHTML=BANK_ACCOUNTS.map(b=>`<div class="bank-card"><strong>${b.bank}</strong><br>Cuenta: ${b.account}<br>CCI: ${b.cci}</div>`).join("")+
      `<button id="transferBtn" class="btn-primary w-full">He transferido</button>`;
    $("#transferBtn").onclick=()=>confirmPayment("Transferencia",total);
  }else if(selectedPaymentMethod==="whatsapp"){
    const msg=`Hola, quiero coordinar pago efectivo. Total ${totalText}.`;
    v.innerHTML=`<a class="btn-primary w-full" target="_blank" href="https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}">Abrir WhatsApp</a>`;
  }
}
...
function prevStep(){if(checkoutStep>1)checkoutStep--;updateStepUI();}
function updateStepUI(){
  ["step1","step2","step3","step4"].forEach((id,idx)=>{$("#"+id)?.classList.toggle("hidden",checkoutStep!==idx+1);});
...
// ---------------------- Modales ----------------------
function openModal(id){$("#"+id)?.classList.add("active");document.body.classList.add("modal-open");}
function closeModal(id){$("#"+id)?.classList.remove("active");document.body.classList.remove("modal-open");}
